...
```{r, include=FALSE}
gas_decice <- quantile(all$MeanDomGas_11_kWh, seq(0.1, 0.9, 0.1), na.rm = TRUE)
elec_decice <- quantile(all$MeanDomElec_11_kWh, seq(0.1, 0.9, 0.1), na.rm = TRUE)
...
```{r, echo=FALSE}
all_sub <- all[all$MeanDomElec_11_kWh > elec_decice[7] | all$MeanDomGas_11_kWh > gas_decice[7] | all$miles_percap > drive_decice[7], ]
```
...
tm_shape(all_sub) +
  tm_polygons(col = "MeanDomElec_11_kWh",
