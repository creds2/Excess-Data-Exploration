...
# Try some models ---------------------------------------------------------
lsoa_model <- lsoa[,!names(lsoa) %in% c("LSOA11CD","dom","SGN","SupergroupCode","SupergroupName","Region","Reg","cluster","cluster2","RUC11CD")]
...
#recode RU
lsoa_mod_DT<-as.data.table(lsoa_model)
lsoa_mod_DT[RU=="A",RUnum:=1];lsoa_mod_DT[RU=="B",RUnum:=2];lsoa_mod_DT[RU=="C",RUnum:=3];lsoa_mod_DT[RU=="D",RUnum:=4];lsoa_mod_DT[RU=="E",RUnum:=5];
lsoa_model<-as.data.frame(lsoa_mod_DT)
...
elec_mat<-as.matrix(lsoa_model[,colnames(lsoa_model) %in% c("RUnum","SupergroupCode","Income","fp10","fpLIHC",
"FullTime","PartTime","SelfEmployed","Student","Unemployed","Retired","Carers","Sick","Old","Young",
"Age_Mean","Age_Median","AB","C1","C2","DE","Single","NoKids",
"Outright","Mortgage","Social","Private","Detached","SemiDetached","Terraced","Flat",
"HHsize","Rooms","NoCH")])

gas_mat<-as.matrix(lsoa_model[,colnames(lsoa_model)%in%c("RUnum","SupergroupCode","Income","fp10","fpLIHC",
"FullTime","PartTime","SelfEmployed","Student","Unemployed","Retired","Carers","Sick","Old","Young",
"Age_Mean","Age_Median","AB","C1","C2","DE","Single","NoKids",
"Outright","Mortgage","Social","Private","Detached","SemiDetached","Terraced","Flat",
"HHsize","Rooms","NoCH")])

car_mat<-as.matrix(lsoa_model[,colnames(lsoa_model) %in% c("RUnum","SupergroupCode","Income",
"FullTime","PartTime","SelfEmployed","Student","Unemployed","Retired","Carers","Sick","Old","Young",
"Age_Mean","Age_Median","AB","C1","C2","DE","Single","NoKids",
"ptte","carte","ptttc","carttc","denonbc","denoffbc","denoffrmt","motor","pt","active","home","AvT2W")])
...
png(filename="plots/iterative/importance_electricity.png")
xgb.plot.importance(importance_m_elec, top_n = 15)
dev.off()

#Remove highest Group
top<-as.character(importance_m_elec[1,1]
elec_mat<-subset(elec_mat, select = -get(top))
lsoa_model$ROOMS<-round(lsoa$Rooms,0)
#lsoaDT<-as.data.table(lsoa_model)
for (i in 3:9){
  t<-subset(lsoa_model, lsoa_model$ROOMS==i)
  t$Rooms<-NULL
  elec_mat<-as.matrix(t[,colnames(t) %in% c("RUnum","SupergroupCode","Income","fp10","fpLIHC",
                                                              "FullTime","PartTime","SelfEmployed","Student","Unemployed","Retired","Carers","Sick","Old","Young",
                                                              "Age_Mean","Age_Median","AB","C1","C2","DE","Single","NoKids",
                                                              "Outright","Mortgage","Social","Private","Detached","SemiDetached","Terraced","Flat","HHsize","NoCH")])
m_elec = xgboost(data = elec_mat, label = lsoa_model$elecAv, nrounds = 10, max_depth = 5)
plot(lsoa_model$elecAv, predict(m_elec, elec_mat)) +   abline(0,1,col = "red")

importance_m_elec = xgb.importance(model = m_elec, feature_names = colnames(elec_mat))
png(filename=paste0("plots/iterative/importance_electricity_Rooms=",i,".png")
xgb.plot.importance(importance_m_elec, top_n = 15)
dev.off()
"HHsize","NoCH")])
