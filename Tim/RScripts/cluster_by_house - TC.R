...
#NB HH Size and Income now removed as 'social'
...
lsoa_house <- lsoa_house[,c("LSOA11CD","elecAv","gasAv",
                            "Detached","SemiDetached","Terraced","Flat","Rooms",
                            "pAge_PRE_1900","pAge_1900_1918","pAge_1919_1929","pAge_1930_1939","pAge_1955_1964",
...
#I had problems getting the fit and groups to work - so just tried a straight kmeans
tclusters <- kmeans(lsoa_clustering, 13)


lsoa_house$hcluster <- tclusters$cluster
...
ggplot(cluster_summary_melt, aes(y = value, x = variable, 
  ...
  geom_line() + 
  labs(x = NULL) +
  ggsave("Tplots/house_hclusters.jpg")
...
ggplot(cluster_summary_melt[cluster_summary_melt$hcluster %in% c(7,11),], aes(y = value, x = variable, 
  ...
  geom_line() + 
  labs(x = NULL) +
  ggsave("Tplots/house_hclusters_c7_11.jpg")


###############################
lsoa_clustering <- lsoa_house[,sapply(lsoa_house,class) %in% c("integer","numeric")]
#lsoa_clustering <- lsoa_clustering[,!names(lsoa_clustering) %in% c("elecAv","gasAv","Income","Rooms","HHsize")]
lsoa_clustering <- lsoa_clustering[,!names(lsoa_clustering) %in% c("elecAv","gasAv")]
...
#Tim plots to look at output


ggplot(lsoa_house, aes(as.character(hcluster),elecAv)) +
  geom_boxplot() +
  labs(x = "Cluster",
       y = "Household Electricity") + 
  ggsave("Tplots/house_cluster_electricity.jpg")

ggplot(lsoa_house, aes(as.character(hcluster), gasAv)) +
  geom_boxplot() +
  labs(x = "Cluster",
       y = "Household Gas") + 
  ggsave("Tplots/house_cluster_gas.jpg")


#Plot the interesting ones
ggplot(cluster_summary_melt[cluster_summary_melt$hcluster %in% c(2,5,11,12),], aes(y = value, x = variable, 
                                                                              group = hcluster, colour = hcluster)) + 
  coord_polar() + 
  #geom_point() + 
  geom_line() + 
  labs(x = NULL) +
  ggsave("Tplots/house_hclusters_c2_5_11_12.jpg")
